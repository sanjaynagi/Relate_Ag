...
rule prep_inputs:
    ...
    shell:
        ...
        qctool_v2.0.7 -filetype shapeit_haplotypes -g {params.haps} -s {input.samples} -incl-samples {params.ox_codes} -og {params.hapsflt} -ofiletype shapeit_haplotypes 2> {log.qctool}
	    awk '{{$1=""}}1' {params.hapsflt} > {params.haps} 2> {log.qctool}
        gzip -c {params.haps} > {output.hapsgz} 2> {log.gzip} && rm {params.hapsflt}
        zgrep -w -F -f {params.ox_codes} {input.samples} > {output.samples} 2> {log.zgrep} 
...
rule Relate:
    ...
    params:
        m = 5.5e-9,
        Ne = 200,
        o_prefix = '{group}_{chrom}',
        poplabels = "data/{group}_{chrom}.poplabels",
        annot = "data/{group}_{chrom}.annot"
    log:
        "logs/Relate/EstimateAncestrees_{group}_{chrom}.log"
    shell:
        """
        Relate --mode All -m {params.m} -N {params.Ne} --haps {input.haps} --sample {input.samples} --map {input.maps} --annot {params.annot} -o {params.o_prefix} 2> {log}
        """

rule demography:
    input:
        mut = '{group}_{chrom}.mut', 
        poplabels = "data/{group}_{chrom}.poplabels"
    output:
